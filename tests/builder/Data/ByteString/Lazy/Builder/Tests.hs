...
testsASCII =
  ...
  , testBuilderConstr "integerDec (large)" (dec_list . amplify) (integerDec . amplify)
  ...
  where
    -- ensure that we also test the code for encoding large integers.
    amplify :: (Integer, Integer, Integer) -> Integer
    amplify (x,y,z) = x ^ (67 :: Integer) - y * x + z

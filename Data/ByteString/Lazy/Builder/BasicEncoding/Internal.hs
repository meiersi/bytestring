{-# LANGUAGE MagicHash, ScopedTypeVariables, CPP, BangPatterns #-}
...
------------------------------------------------------------------------------
-- An unboxed representation of IO (Ptr Word8)
------------------------------------------------------------------------------


#if defined(__GLASGOW_HASKELL__)

-- | Unboxed representation of @IO (Ptr Word8)@.
type IO_Addr = State# RealWorld -> (# State# RealWorld, Addr# #)

-- | Convert an @IO (Ptr Word8)@ action to an 'IO_Addr'.
{-# INLINE CONLIKE toIO_Addr #-}
toIO_Addr :: IO (Ptr Word8) -> IO_Addr
toIO_Addr (IO io) w = case io w of (# w', Ptr addr #) -> (# w', addr #)

-- | Convert an 'IO_Addr'to and @(Ptr Word8)@ action.
{-# INLINE CONLIKE fromIO_Addr #-}
fromIO_Addr :: IO_Addr -> IO (Ptr Word8)
fromIO_Addr f = IO (\w -> case f w of (# w', addr #) -> (# w', Ptr addr #))

-- | Bind two 'IO_Addr' actions.
{-# INLINE CONLIKE bindIO_Addr #-}
bindIO_Addr :: IO_Addr -> (Ptr Word8 -> IO_Addr) -> IO_Addr
bindIO_Addr m f = \w -> case m w of (# w', addr #) -> f (Ptr addr) w'

#else

ERROR

-- Stubs for compilers other than GHC

type IO_Addr = IO (Ptr Word8)
...
data BoundedEncoding a = BE {-# UNPACK #-} !Int (a -> Ptr Word8 -> IO_Addr)
...
-- /must/ be referentially transparent in the sense that it always encodes a
-- value to the same sequence of bytes.
{-# INLINE CONLIKE boundedEncoding #-}
boundedEncoding 
    :: Int 
...
runB (BE _ io) x op = fromIO_Addr $ io x op
